  ...
  export const Loading = () => {
    return (
      <div className="flex items-center justify-center max-h-screen mt-36">
        <div className="relative">
          <div className="animate-spin rounded-full h-16 w-16 border-t-4 bg-indigo-600"></div>
          <div className="absolute top-1 left-1 animate-spin rounded-full h-14 w-14 border-b-4 border-indigo-400" style={{animationDirection: 'reverse', animationDuration: '1.2s'}}></div>
          <div className="absolute top-3 left-3 animate-pulse rounded-full h-10 w-10 bg-indigo-100 opacity-50"></div>
        </div>
      </div>
    );
  };
